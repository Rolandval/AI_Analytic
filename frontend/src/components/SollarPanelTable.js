...
const SollarPanelTable = ({ sollarPanels, onChartClick, loading }) => {
  ...
  if (!sollarPanels || sollarPanels.length === 0) {
    return (
      <Card sx={{ mt: 2, mb: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
        <CardHeader
          title={
    ...
    <Card sx={{ mt: 2, mb: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
      <CardHeader
        title={
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <TableChartIcon sx={{ mr: 1, color: theme.palette.primary.main }} />
            <Typography variant="h6" sx={{ fontWeight: 600 }}>
              Результати пошуку ({sollarPanels.length})
            </Typography>
          </Box>
        }
        action={
      ...
      {expanded && (
        <TableContainer component={Paper} sx={{ boxShadow: 'none' }}>
          <Table sx={{ minWidth: 650 }} aria-label="solar panels table">
            <TableHead sx={{ backgroundColor: alpha(theme.palette.primary.main, 0.04) }}>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Потужність (Вт)</TableCell>
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Тип панелі</TableCell>
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Тип клітин</TableCell>
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Товщина (мм)</TableCell>
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Ціна</TableCell>
                <TableCell sx={{ fontWeight: 600, whiteSpace: 'nowrap' }}>Ціна за Вт</TableCell>
            ...
            <TableBody>
              {sollarPanels.map((panel) => (
                ...
                >
                  ...
                  <TableCell>{panel.supplier}</TableCell>
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <ElectricBoltIcon sx={{ fontSize: 16, mr: 0.5, color: theme.palette.warning.main }} />
                  ...
                  <TableCell>
                    <Chip 
                      label={panel.panel_type || 'Не вказано'} 
                      size="small" 
                      variant="outlined"
                      sx={{ fontWeight: 500 }}
                    />
                  ...
                  <TableCell>
                    <Chip 
                      label={panel.cell_type || 'Не вказано'} 
                      size="small" 
                      variant="outlined"
                      sx={{ fontWeight: 500 }}
                    />
                  </TableCell>
                  <TableCell>{panel.thickness} мм</TableCell>
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
...
export default SollarPanelTable;
