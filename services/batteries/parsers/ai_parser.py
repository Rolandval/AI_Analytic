...
- `price`: найменший оптовий, ціна завжди має бути(якщо немає то 0) постарайся проаналізувати з рядка яке значення є найближчим
- `name`: назва акумулятора між брендом і об'ємом(якщо пустий рядок то поверни в name назву бренду)
- `c_amps`: пусковий струм(якщо немає то 0) якщо не вказано ніяких букв то подумай логічно яке з цих значень найбільш наближене і реальне до пускового струму
