...
- `name`: назва панелі між брендом і потужністю (якщо відсутня — просто поверни бренд)
- `power`: потужність у ватах (W). Якщо немає явного значення — спробуй логічно вивести його з назви моделі або повного опису. Якщо не вдається — поверни 0.
- `full_name`: повна назва або опис панелі
- `price`: найменша оптова ціна у доларах США за 1 штуку. Якщо ціна вказана за ват (тобто підозріло мала, менше 1$), перемнож на потужність панелі. Якщо ціна в іншій валюті — переведи у долари. Якщо немає ціни — поверни 0.
- `panel_type`: одностороння або двостороння. Якщо не вказано — одностороння.
- `cell_type`: n-type або p-type. Якщо не вказано — n-type.
- `thickness`: товщина панелі у міліметрах. Якщо не вказано — поверни 30.

‼️ВАЖЛИВО:
- Ціна формату "0.13", "0,138" — це ціна за ват! Не сплутай з 13 доларами.
  Наприклад: потужність 425W * ціна 0.13 = 55.25$
